...
func (suite *ContextConfigurationTestSuite) SetupTest() {
	suite.ctxConfiguration = NewContextConfiguration(suite.cfg, suite.logger, suite.js)
}
...
func (suite *ContextConfigurationTestSuite) TestSetProjectConfig() {
	...
	// to project bucket
	err := suite.ctxConfiguration.SetConfig(key, value, ScopeProject)
	suite.Require().NoError(err)
	savedValue := suite.getValueFromKVStore(suite.cfg.NATS.KeyValueStoreProjectName, key)
	suite.Assert().Equal(value, savedValue)

	// to workflow bucket
	err = suite.ctxConfiguration.SetConfig(key, value, ScopeWorkflow)
	suite.Require().NoError(err)
	...
	// to node bucket
	value = "value2"
	err = suite.ctxConfiguration.SetConfig(key, value, ScopeNode)
	suite.Require().NoError(err)
	savedValue = suite.getValueFromKVStore(suite.cfg.NATS.KeyValueStoreNodeName, key)
	suite.Assert().Equal(value, savedValue)

	// to deafult bucket (node)
	err = suite.ctxConfiguration.SetConfig(key, value)
	suite.Require().NoError(err)
	savedValue = suite.getValueFromKVStore(suite.cfg.NATS.KeyValueStoreNodeName, key)
	suite.Assert().Equal(value, savedValue)
}
...
func (suite *ContextConfigurationTestSuite) TestGetProjectConfig() {
	...
	err := suite.ctxConfiguration.SetConfig(key, value, ScopeProject)
	suite.Require().NoError(err)
	savedValue, err := suite.ctxConfiguration.GetConfig(key, ScopeProject)
	suite.Require().NoError(err)
	suite.Assert().Equal(value, savedValue)
	...
	err = suite.ctxConfiguration.SetConfig(key, value, ScopeWorkflow)
	suite.Require().NoError(err)
	savedValue, err = suite.ctxConfiguration.GetConfig(key, ScopeWorkflow)
	suite.Require().NoError(err)
	suite.Assert().Equal(value, savedValue)
	...
	err = suite.ctxConfiguration.SetConfig(key, value, ScopeNode)
	suite.Require().NoError(err)
	savedValue, err = suite.ctxConfiguration.GetConfig(key, ScopeNode)
	suite.Require().NoError(err)
	suite.Assert().Equal(value, savedValue)
	...
	err = suite.ctxConfiguration.SetConfig(key, value)
	suite.Require().NoError(err)
	savedValue, err = suite.ctxConfiguration.GetConfig(key)
	suite.Require().NoError(err)
	suite.Assert().Equal(value, savedValue)
}
...
func (suite *ContextConfigurationTestSuite) TestDeleteProjectConfig() {
	...
	err := suite.ctxConfiguration.SetConfig(key, value, ScopeProject)
	suite.Require().NoError(err)
	err = suite.ctxConfiguration.DeleteConfig(key, ScopeProject)
	suite.Require().NoError(err)
	_, err = suite.ctxConfiguration.GetConfig(key, ScopeProject)
	suite.Require().Error(err)

	// from workflow bucket
	err = suite.ctxConfiguration.SetConfig(key, value, ScopeWorkflow)
	suite.Require().NoError(err)
	err = suite.ctxConfiguration.DeleteConfig(key, ScopeWorkflow)
	suite.Require().NoError(err)
	_, err = suite.ctxConfiguration.GetConfig(key, ScopeWorkflow)
	suite.Require().Error(err)

	// from node bucket
	err = suite.ctxConfiguration.SetConfig(key, value, ScopeNode)
	suite.Require().NoError(err)
	err = suite.ctxConfiguration.DeleteConfig(key, ScopeNode)
	suite.Require().NoError(err)
	_, err = suite.ctxConfiguration.GetConfig(key, ScopeNode)
	suite.Require().Error(err)

	// from default bucket (node)
	err = suite.ctxConfiguration.SetConfig(key, value)
	suite.Require().NoError(err)
	err = suite.ctxConfiguration.DeleteConfig(key)
	suite.Require().NoError(err)
	_, err = suite.ctxConfiguration.GetConfig(key)
	suite.Require().Error(err)
}
